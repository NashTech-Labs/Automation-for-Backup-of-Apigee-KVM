...
pipeline {
    ...
    stages {
        ...
        stage("Exporting kvms from Apigee") {
            steps {
            script {
            def currentDate = new Date().format("dd-MM-yyyy")
                    def exportDir = "<APIGEE_ORG_NAME>/${currentDate}/kvms"
                    
                    sh """
                    mkdir -p ${exportDir}
                    ${APIGEE_CLI_DIR}/apigeecli kvms export -o <APIGEE_ORG_NAME> -t ${TOKEN} 
                    """
                    sh "mv kvm.json ${exportDir}"
            ...
            if (changes) {
                sh "git commit -m 'Added kvms from Jenkins Pipeline'"
                withCredentials([gitUsernamePassword(credentialsId: "${GITHUB_CREDS}", gitToolName: 'Default')]) {
                    sh "git push -u origin main"
                }
            } else {
                echo "No changes to commit in kvms. Skipping git push."
                    }
                }
            }
        }
